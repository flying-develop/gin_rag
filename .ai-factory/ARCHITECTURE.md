...
│                                        # вне modules/ и infrastructure/, чтобы оба слоя
│                                        # могли зависеть от него без нарушения правил
│
...
- Транзакции инициируются на уровне сервиса через `db.WithinTx(ctx, db, fn)`:
  хелпер кладёт `*gorm.DB` транзакции в `ctx`, а репозитории достают её
  через `db.Conn(ctx, fallback)`. Поэтому операции внутри `fn` идут в одной
  транзакции без передачи `tx` явным аргументом и без метода `WithTx` в
  интерфейсе репозитория.
- Доменные ошибки — значения `*apperr.Error` с категорией `Kind`
  (`NotFound`/`Validation`/`Conflict`/`Internal`). Сервис возвращает их
  наверх, `errorHandler` в `httpserver` сопоставляет `Kind` с HTTP-кодом.
